...
class MapHandler {
    ...
    reverseMoveArray() {
        let reverseArray = this.moveArray.reverse();
        this.lastMoveIndex = reverseArray.length - 1;
        return reverseArray;
    }
    makeMoveArray(yStart, xStart) { //does it write it down normally? sometimes it works as expected, sometimes it's array islonger than needed, usually when i press startgame two times in a row, same level
    ...
    this.moveArray[0] = {x: xStart, y: yStart}; 
    let lastIndex = this.moveArray.length - 1;
    ...
    while (minWaveValue != 0 && count < 64) { //until we reach the end point (which will be in the last array element)
        let x = this.moveArray[lastIndex].x;
        let y = this.moveArray[lastIndex].y;
        ...
        if (minX != -1 && minY != - 1) {
            this.moveArray.push({x: minX, y: minY});
        }
        lastIndex = this.moveArray.length - 1;
        count++;
    } 
    this.firstMove = false;
    this.lastMoveIndex = this.moveArray.length - 1;
...
class Point {//console.log(Point.distance(p1, p2));
  ...
  class Move {
    constructor(gameState) {
        this.gameState = gameState;
        this.shipX = gameState.ship.x + 1;
        this.shipY = gameState.ship.y + 1;
        this.ports = gameState.ports;
        this.goodsInPort = gameState.goodsInPort;
       // this.shipGoods = gameState.ship.goods;
    }
    isInPort(isHome) {//assuming home is in first, home is true 
        let startIndex = isHome ? 0 : 1;
        let endIndex = isHome ? 1 : this.ports.length;
        let flag = false;
    
        for (let portId = startIndex; portId < endIndex; portId++) {
            let portX = this.ports[portId].x + 1;
            let portY = this.ports[portId].y + 1;
            if (this.shipX == portX && this.shipY == portY) { //+1 padding???
                flag = true;
            }
        }
        return flag;
    }
    canLoad() { //Если корабль попробует загрузить товар, которого нет в домашнем порту, упрощенная версия
        let available = false; //is there stuff to load from the port
        let shipGoods = this.gameState.ship.goods;
        for (let i = 0; i < this.goodsInPort.length; i++){
            if  (this.goodsInPort[0].amount > 0) {
                available = true;
            }
        }
        console.log("SG LEN" + shipGoods.length);
    ...
    canSell() {//sell onl if there is something to sell
        return this.shipGoods.length != 0
    }
}
...
export function getNextCommand(gameState) {
    ...
    console.log("here sg len" + shipGoods.length);
    if (move.isInPort(true) || move.isInPort(false)) {
        if (move.isInPort(true) && move.canLoad()) {//is in homeport
            let portId = market.bestPort(); 
                ...
                if (portId != market.getLastBestPortId()) {//there is a new bestport now
                    ...
                    //map.makeMoveArray(ports[portId].y + 1, ports[portId].x + 1); //calculating new route using same wave данные
                    //REUSE SAME ARRAY WITHOUT POP
                    //IF CAME BACK HOME FROM PORT JUST REVERSE, else no need to reverse it
                    if (!map.isFirstMove()) {
                        map.reverseMoveArray();
                    }
                    
                } 

            let i = market.profitIndex(portId); //the product index we want to load
            let load = market.loadAmount(goodsInPort[i]) //REFACTOR СКОБКИ
            console.log("are we there yet?");
            return "LOAD " + goodsInPort[i].name + " " + load; 
        } else {
            if  (isInPort(false) && canSell()) {
                map.reverseMoveArray();////calculating new route (to home)
                //map.makeMoveArray(moveArray[last].y, moveArray[last].x).reverse();//calculating new route (to home)
